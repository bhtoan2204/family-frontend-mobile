...
import {selectLocale, getTranslate} from 'src/redux/slices/languageSlice';
              ...
              {iconList.map((icon, index) => {
                if (index > 0) {
                  return (
                    <TouchableOpacity
                      key={index}
                      className="flex-row items-center justify-between mr-3 mb-2"
                      onPress={() => {
                        setPickIcon(icon);
          ...
          <View className="flex-1  mt-10">
            ...
            <View className=" items-center">
              <Text className="text-base font-semibold text-[#2A475E] dark:text-white">
                {translate('checklist_screen_add_checklist_title')}
              </Text>
              <Text className="text-sm my-3  text-[#2A475E] dark:text-[#8D94A5]">
                {translate('checklist_screen_add_checklist_description')}
              </Text>
            </View>
            {buildInputName()}
            {buildPickIcon()}
