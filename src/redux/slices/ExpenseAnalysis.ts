  ...
  reducers: {
    ...
    updateExpense: (state, action: PayloadAction<DailyExpense>) => {
      const index = state.expenses.findIndex(
        expense => expense.id_expenditure === action.payload.id_expenditure,
      );
      ...
      if (
        ...
        state.selectedExpense.id_expenditure === payload.id_expenditure
      ) {
    ...
    deleteExpense: (state, action: PayloadAction<number>) => {
      const expenseToRemove = state.expenses.find(
        expense => expense.id_expenditure === action.payload,
      );
      if (expenseToRemove) {
      ...
      if (
        state.selectedExpense &&
        state.selectedExpense.id_expenditure === action.payload
      ) {
