...
import { EditConsumableHouseHoldItemScreenProps, EditExpenseHouseHoldItemScreenProps } from 'src/navigation/NavigationTypes'
...
const EditConsumableHouseHoldItemScreen = ({ navigation, route }: EditConsumableHouseHoldItemScreenProps) => {
    const { id_family, id_item, expired_date, id_category, quantity, threshhold } = route.params
    ...
    const [quantityInput, setQuantityInput] = React.useState<number>(quantity)
    const [threshholdInput, setThreshholdInput] = React.useState<number>(threshhold)
    const [expiredDateInput, setExpiredDateInput] = React.useState<Date>(new Date(expired_date))
    ...
    const handleCancel = () => {
        ...
        setThreshholdInput(threshhold)
        setExpiredDateInput(new Date(expired_date))
    }
                    ...
                    <View className='w-full  flex-row justify-between items-center py-3 '>
                        {
                            (quantityInput != consumableData?.quantity || threshholdInput != consumableData?.threshold || expiredDateInput.toDateString() != consumableData?.expired_date) ? <>
                                ...
                                <Text className='text-base font-medium  '>Edit consumable item</Text>
                                <View className='mr-3'>
                                    <TouchableOpacity onPress={() => {
                                        handleSave()
                                    }} >
...
export default EditConsumableHouseHoldItemScreen