...
import { GuildLineScreenProps, SharedGuidelineScreenProps } from 'src/navigation/NavigationTypes'
...
const ShareGuildLineScreen: React.FC<SharedGuidelineScreenProps> = ({ navigation, route }) => {
    ...
    const fetchGuidelines = React.useCallback(async () => {
        try {
            setLoading(true);
            const response = await GuildLineService.getSharedGuidelines(page, itemsPerPage); // API call to fetch all guidelines
    ...
    const renderEmptyGuideline = () => {
        return <View>
            <Text className='text-center text-lg font-semibold' style={{ color: COLORS.AuroMetalSaurus }}>No shared guideline</Text>
        </View>
    }
                ...
                <View>
                    {
                        guidelines.length > 0 ? guidelines.map((item, index) => (
                            <React.Fragment key={index}>
                                <SharedGuildlineItem item={item} index={index}
                                    onPress={() => {
                                        navigation.navigate('SharedGuidelineDetail', { id_guide_item: item.id_guide_item, id_family: item.id_family })
                                    }}
                                />
                            </React.Fragment>
