...
const SubjectDetailScreen: React.FC<SubjectDetailScreenProps> = ({
  navigation,
  route,
}) => {
  const {id_education_progress, id_family, id_subject} = route.params;
  const [subjectDetailData, setSubjectDetailData] = React.useState<Subject>(
    data.data[0],
  );
    ...
    const handleCalculateScore = () => {
      ...
      if (subjectDetailData.final_score != null) {
        if (subjectDetailData.final_score.score != null) {
          ...
          totalScore += parseFloat(
            (subjectDetailData.final_score.score || 0).toString(),
          );
        }
        if (subjectDetailData.final_score.expected_score != null) {
          expectedCount += 1;
          totalExpectedScore += parseFloat(
            (subjectDetailData.final_score.expected_score || 0).toString(),
          );
        }
      }
      if (subjectDetailData.midterm_score != null) {
        if (subjectDetailData.midterm_score.score != null) {
          scoreCount += 1;
          totalScore += parseFloat(
            (subjectDetailData.midterm_score.score || 0).toString(),
          );
        }
        if (subjectDetailData.midterm_score.expected_score != null) {
          expectedCount += 1;
          totalExpectedScore += parseFloat(
            (subjectDetailData.midterm_score.expected_score || 0).toString(),
          );
        }
      }
      ...
      setCurrentGrade(
        parseFloat((totalScore / scoreCount).toPrecision(2)) || 0,
      );
      setExpectedGrade(
        parseFloat((totalExpectedScore / expectedCount).toPrecision(2)) || 0,
      );
    };
      ...
      <View className="flex-1 bg-[#F7F7F7]">
        <View className="w-full  flex-row justify-between items-center py-3 bg-white">
          ...
          <View>
            <Text
              className="text-lg font-semibold"
              style={{color: COLORS.primary}}>
              {getFirstLetterSubject(data.data[0].subject_name)}
            </Text>
          </View>
          <View className="mr-3">
            <TouchableOpacity
        ...
        <View className="h-28 bg-white border-b-[1px] border-gray-200">
          <View className="flex-row h-full p-2 bg-[#FFFFFE]">
            <View className="flex-1 border-r-[1px] border-gray-200  flex-col  items-center">
              <Text
                className="text-base  text-opacity-0 font-medium"
                style={{color: COLORS.primary}}>
                Current grades
              </Text>
              <View className="mt-2">
        ...
        <ScrollView className=" ">
          <View className="my-3">
            ...
            <SubjectItem
              isGraded={subjectDetailData.midterm_score?.score != null}
              subjectComponentData={subjectDetailData.final_score}
              setSubjectDetailData={setSubjectDetailData}
              index={-1}
            />
            <SubjectItem
              isGraded={subjectDetailData.midterm_score?.score != null}
              subjectComponentData={subjectDetailData.midterm_score}
              setSubjectDetailData={setSubjectDetailData}
              index={-2}
            />
          </View>
