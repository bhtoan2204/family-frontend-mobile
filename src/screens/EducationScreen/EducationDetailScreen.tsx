...
const EducationDetailScreen: React.FC<EducationDetailScreenProps> = ({
  navigation,
  route,
}) => {
  const {id_education_progress, id_family} = route.params;
  const [educationDetailData, setEducationDetailData] =
    React.useState<EducationDetail>(data.data[0]);
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: '#fff'}}>
      <View className="flex-1 bg-[#F7F7F7]">
        ...
        <ScrollView className=" ">
          {educationDetailData &&
            educationDetailData.subjects_info.map(
