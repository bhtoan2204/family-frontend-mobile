...
import {deleteTodoLists, doneTodoList} from 'src/redux/slices/CalendarSlice';

const screenHeight = Dimensions.get('screen').height;

const TodoListCategoryDetailScreen = ({
  navigation,
  route,
}: TodoListItemDetailScreenProps) => {
  const {id_family, id_category, id_item} = route.params;
  const dispatch = useDispatch<AppDispatch>();
  const itemDetail = useSelector((state: RootState) => state.todoList)
    .todoListType.find(item => item.id_checklist_type == id_category)
    ?.checklists.find(item => item.id_checklist == id_item);
          ...
          <View className=" flex-row mr-2 items-center">
            <TouchableOpacity
              ...
              style={{
                ...
                borderColor: itemDetail?.is_completed
                  ? 'transparent'
                  : '#CBCBCB',
                backgroundColor: itemDetail?.is_completed
                  ? '#00AE00'
                  : undefined,
              }}
              onPress={() => {
        ...
        <View className="flex-row  items-center  w-full  py-2 ">
          <View className="mr-2">
            <Material
              ...
              size={30}
              color={colorScheme == 'light' ? '#5D5D5D' : 'white'}
            />
          </View>

          <Text className="text-base text-[#2F2F34] dark:text-white">
            {itemDetail?.due_date
              ? format(new Date(itemDetail?.due_date), 'dd/MM/yyyy')
              : 'Set reminder date'}
          </Text>
        </View>
      </TouchableOpacity>
    ...
    Alert.alert('Delete item', 'Are you sure you want to delete this item?', [
      {
        ...
        onPress: async () => {
          ...
          dispatch(deleteTodoLists({id_item: id_item}));
          // toast.show("Deleted", {
          //     type: "success",
          //     duration: 2000,
          //     icon: <Material name="check" size={24} color={"white"} />,
  ...
  return (
    <View
      ...
      <UpdateDateItemSheet
        ...
        id_list={id_category}
        initialDate={
          itemDetail?.due_date ? itemDetail?.due_date : new Date().toISOString()
        }
        onUpdateSuccess={() => {
