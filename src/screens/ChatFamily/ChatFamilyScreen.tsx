...
import { StyleSheet, Text, View, TextInput, Button, ScrollView, TouchableOpacity, KeyboardAvoidingView, Platform } from 'react-native';
...
const ChatFamilyScreen = ({ navigation, route }: ChatFamilyScreenProps) => {
  ...
  const fetchMessages = async (index: number) => {
    try {
      console.log(id_user);
      const response = await ChatServices.GetFamilyMessages({ id_family: id_family, index: index });
      if (response) {
          ...
          messages.reverse().map((msg, index) => (
            <View
              ...
              style={[
                ...
                msg.senderID === id_user ? styles.receiverContainer : styles.senderContainer
              ]}
            >
              {msg.senderID !== id_user && members.length > 0 && (
                <TouchableOpacity>
                  <Text style={styles.senderName}>{`${members[0].firstname} ${members[0].lastname}`}</Text>
                </TouchableOpacity>
              )}
