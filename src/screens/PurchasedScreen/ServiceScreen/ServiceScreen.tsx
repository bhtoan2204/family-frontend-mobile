  ...
  const fetchServiceOfFamily = async (id_family: number) => {
    ...
    setLoading(true); 
      ...
      if (data) {
        ...
        const purchasedIds = data.map((service: { id_extra_package: any; }) => service.id_extra_package);
...
<View style={{ flexDirection: 'row' }}>
  ...
  >
   <Icon name="add-circle-outline" size={50} color="#007AFF" style={{ marginRight: 5,  }} /> 
  ...
  <FlatList
    ...
    renderItem={({ item }) => (
      <TouchableOpacity
        style={[
          styles.familyCard,
          familySelected?.id_family === item.id_family && styles.familyCardSelected,
        ]}
        ...
        <View style={styles.serviceList}>
          ...
          {service.map((pkg, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.serviceItem,
                selectedService?.id_extra_package === pkg.id_extra_package && styles.serviceItemActive,
              ]}>
              <Text style={[purchasedServices.includes(pkg.id_extra_package) && styles.serviceNamePur, styles.serviceName]}>{pkg.name}</Text>
              ...
              <View style={styles.serviceActions}>
                {purchasedServices && purchasedServices.includes(pkg.id_extra_package) ? (
