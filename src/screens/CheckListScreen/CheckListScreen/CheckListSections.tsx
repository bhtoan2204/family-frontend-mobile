...
const ChecklistSections: React.FC<{
  ...
  selectedCategory,
  setSelectedCategory,
  handleNavigateCheckListDetail,
  searchString,
  setSearchString,
  bottomSheetRef,
}) => {
  const [sections, setSections] = React.useState<
    {title: string; data: CheckListCategoryInterface[]}[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    checklist.forEach(item => {
      ...
      if (compareDates(dueDate, today) === 0) {
        const todayIndex = newSections.findIndex(
        ...
        const overdueIndex = newSections.findIndex(
          section => section.title === 'Overdue',
        );
        if (overdueIndex === -1) {
        ...
        const dateIndex = newSections.findIndex(
