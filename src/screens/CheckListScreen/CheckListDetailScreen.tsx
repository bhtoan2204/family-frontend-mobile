...
const ChecklistDetailScreen: React.FC<CheckListDetailScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const checkListCategoryData = useSelector(
    (state: RootState) => state.checklist,
  ).find(item => item.id === id_checklist)!;
  const checklist = useSelector((state: RootState) => state.checklist).find(
    item => item.id === id_checklist,
  )!.checklistItems;
  // const [checklist, setChecklist] = React.useState<ChecklistItemInterface[]>(checklistData);
  const [filteredChecklist, setFilteredChecklist] =
    React.useState<ChecklistItemInterface[]>();
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
      <View style={styles.container}>
        ...
        <ScrollView
          className="flex-1 rounded-t-xl mt-[-10] pt-3  bg-white"
          showsVerticalScrollIndicator={false}>
          <View
            className="pl-3 mt-2 py-3 mb-8"
            style={{
              backgroundColor:
                shoppingListItemColorInside[
                  checkListCategoryData.id_item_type - 1
                ],
            }}>
            <Text className="text-xl font-bold mb-4">
              {checkListCategoryData.title}{' '}
            </Text>
            <View
              style={{
                backgroundColor:
                  shoppingListItemColor[checkListCategoryData.id_item_type - 1],
            ...
            <View className="flex-row items-center">
              <View className="mr-2">
                <CircularProgress
                  size={20}
                  progress={
                    (checkListCategoryData.completed /
                      checkListCategoryData.total) *
                    100
                  }
                  strokeWidth={2}
              ...
              <Text className="text-[#908B89] text-sm">
                {checkListCategoryData.completed}
              </Text>
        ...
        <TouchableOpacity
          ...
          className="absolute bottom-6 right-4 bg-gray-200 h-16 w-16 flex-row items-center justify-center rounded-full"
          style={{
            backgroundColor:
              shoppingListItemColor[checkListCategoryData.id_item_type - 1],
          }}>
