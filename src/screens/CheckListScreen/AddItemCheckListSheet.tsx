  ...
  return (
    <RBSheet
      ...
      <View className="flex-row items-center my-5">
        <TouchableOpacity
          className=" ml-4 px-4 py-3 flex-row justify-center items-center border-[1px] border-[#EAEAEA] rounded-lg "
          onPress={() => {
            console.log(1);
            onPressPriority();
          }}>
          {priority == 4 ? (
            <Material
              name="flag-outline"
              size={20}
              style={{color: priorityColors[priority - 1], fontWeight: 'bold'}}
...
export const TimePickerSheet = ({
  ...
  refRBSheet: React.RefObject<any>;
  setSave: (dueDate: Date | null) => void;
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    initialValue != null
      ? (initialValue!.getMonth() + 1).toString().padStart(2, '0')
      : (new Date().getMonth() + 1).toString().padStart(2, '0'),
  );
  const [selectedDay, setSelectedDay] = useState(
    initialValue?.getDate().toString().padStart(2, '0') ||
      ...
      <View>
        <View className="w-full  flex-row justify-between  items-center py-3 bg-white px-4 z-10">
          <TouchableOpacity
            ...
            <Text className="text-base font-medium" style={{color: COLORS.red}}>
              Cancel
            </Text>
          </TouchableOpacity>
          <View className=" ">
        ...
        <View className="flex-row items-center">
          <Picker
            ...
            {[...Array(31).keys()].map(day => (
              <Picker.Item
                key={day}
                label={`${day + 1}`.padStart(2, '0')}
                value={`${day + 1}`.padStart(2, '0')}
              />
            ))}
          </Picker>
