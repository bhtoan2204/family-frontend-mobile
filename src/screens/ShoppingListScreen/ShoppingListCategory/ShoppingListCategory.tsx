...
const ShoppingListCategoryScreen = ({
  navigation,
  route,
  ...
  const shoppingListInfo = useSelector(
    (state: RootState) => state.shoppinglist,
  ).shoppingList.filter(item => item.id_shopping_list_type === id_category);
  const shoppingListType = useSelector(
    (state: RootState) => state.shoppinglist,
  ).shoppingListType;
  console.log(shoppingListInfo);
  const items: Map<string, ShoppingListItem[]> = React.useMemo(
    () => mapByItemType(shoppingListInfo[0]?.items || []),
    [shoppingListInfo],
  );
  ...
  const getCategoryName = React.useCallback(
    (id_category: number) => {
      const category = shoppingListType.find(
        item => item.id_shopping_list_type === id_category,
      );
      if (category) {
  ...
  const handleNavigateItemDetail = React.useCallback(
    (id_list: number, id_item: number) => {
  ...
  const handleCompleteItem = React.useCallback(
    async (id_list: number, item: ShoppingListItem) => {
    ...
    return (
      <View className="flex-1 justify-center items-center bg-white dark:bg-[#0A1220] ">
        <View className="justify-center items-center mt-[-4%]">
          ...
          <Text className="text-[#747474] my-2 font-bold text-lg dark:text-[#d8d8d8]">
            Nothing to buy?
          </Text>
          <Text className="mx-[15%] text-center text-sm text-[#747474] dark:text-[#d8d8d8]">
  ...
  const buildItems = React.useCallback(() => {
    return Array.from(items.entries()).map(([item, index]) => {
      return (
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#f7f7f7'}}>
        ...
        <View className="bg-white flex-1 ">
          {loading ? (
            ...
            <>
              {items && items.size > 0 ? (
                <ScrollView
                  className="flex-1 bg-white dark:bg-[#0A1220]"
      ...
      />
      <ShoppingListPickCategorySheet
        refRBSheet={addCategoryBottomSheetRef}
        categories={categories}
